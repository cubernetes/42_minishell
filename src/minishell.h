...
t_list			*handle_special_param(t_token **tok, t_token *token, int *start, int idx, int *parse_param);
t_list			*handle_end_of_param(t_token **tok, t_token *token, int *start, int idx, int *parse_param);
t_list			*handle_param(t_token **tok, t_token *token, int *start, int idx, int *parse_param);
t_list			*split_into_subwords(t_token *token);
t_token			*new_word_token(t_token *token, int start, int end);
t_list			*handle_ifs_whitespace(bool *push, t_token *token, int *start, int idx);
t_list			*handle_ifs_non_whitespace(bool *push, t_token *token, int *start, int idx);
t_list			*handle_ifs(bool *push, t_token *token, int *start, int idx);
t_list			*handle_expansion_ctx(bool *push, t_token *token, int *start, int idx);
t_list			*push_words(bool *push, t_token *token, int *start, int idx);
t_list			*handle_post_splitting(bool push, t_token *token, int start, int idx);
t_list			*split_into_words(t_token *token);
t_token			*ft_tilde_expand(t_token *token);
t_token			*gen_joined_token(t_list *subwords);
void			handle_word_token(t_token *joined_token, t_list *new_tokens, t_list *tokens);
void			handle_dquote_str_token(t_token *joined_token, t_list *new_tokens, t_list *tokens);
t_list			*expand_tokens(t_list *tokens);
void			join_word_token(t_token *word_token[static 1], t_token *token);
void			join_or_acc(t_token *token, t_list *new_tokens, t_token *word_token[static 1]);
...
void			interactive_interrupt(int sig);void noninteractive_interrupt(int sig);void noninteractive_quit(int sig);void interactive_interrupt_heredoc(int sig);unsigned char get_last_exit_status(void);
...
t_ht			**var_manager(char *key_or_ret[static 1], char *value_or_key, t_flags flags);
...
bool			sort_vars(t_data data1, t_data data2);int add_var_flags(char key[static 1], char *value, t_declare_flags flags, t_var *orig_var);char *flags_to_str(t_var *var);bool force_ansi_c_quoting(char *s);
...
bool			sort_vars(t_data data1, t_data data2);int add_var_flags(char key[static 1], char *value, t_declare_flags flags, t_var *orig_var);char *flags_to_str(t_var *var);/* bare_declare will be removed in the future when this function is * actually implemented */char *quote(char *s, bool bare_declare);
...
int				add_var_flags(char key[static 1], char *value, t_declare_flags flags, t_var *orig_var);
...
void			prepare_stack(t_list *stack[static 1]);bool starts_with_bang(t_list *tokens);bool terminates_pipe_sequence(t_list *tokens);bool handle_ast_node(t_tree *tree[static 1], t_tree top[static 1], t_list tokens[static 1], t_list stack[static 1]);int handle_token( t_tree *tree[static 1], t_tree top[static 1], t_list tokens[static 1], t_list stack[static 1]);t_list *build_as_recursively(t_tree *tree, bool mk_heredoc);bool is_unimportant_token(t_token *token);bool is_proper_ast_node(t_tree *tree);bool is_improper_ast_node(t_tree *tree);void flatten_redirect(t_tree *first, t_list *chldn, t_list *children, bool mk_heredoc);void handle_proper_ast_node(t_tree *tree, t_list *flat, bool mk_heredoc);/* LL(1) parser */t_tree *build_parse_tree(t_list tokens[static 1]);
void			handle_improper_ast_node(t_tree *tree, t_list *flat, bool mk_heredoc);
t_list			*build_as_recursively(t_tree *tree, bool mk_heredoc);
t_tree			*build_ast(t_list *tokens, bool mk_heredoc);
t_list			*build_as_recursively(t_tree *tree, bool mk_heredoc);bool is_unimportant_token(t_token *token);
bool			is_proper_ast_node(t_tree *tree);
bool			is_improper_ast_node(t_tree *tree);
void			flatten_redirect(t_tree *first, t_list *chldn, t_list *children, bool mk_heredoc);
...
bool			terminates_pipe_sequence(t_list *tokens);
bool			handle_ast_node(t_tree *tree[static 1], t_tree top[static 1], t_list tokens[static 1], t_list stack[static 1]);
int				handle_token(t_tree *tree[static 1], t_tree top[static 1], t_list tokens[static 1], t_list stack[static 1]);
...
bool			handle_interactive_heredoc(int fd, char *ps2, char *delimiter, char *input[static 1]);
bool			handle_noninteractive_heredoc(int fd, char *ps2, char *delimiter, char *input[static 1]);
...
char			*get_krb_ticket_file(char *uid);char *get_username_from_krb_file(int fd);char *get_username_from_etc_passwd(char *uid);
