...
t_list							*handle_dollar(t_token **tok, t_token *token,
									int *start, int idx);
t_list							*handle_special_param(t_token **tok,
									t_token *token, int *start, int idx,
									int *parse_param);
t_list							*handle_end_of_param(t_token **tok,
									t_token *token, int *start, int idx,
									int *parse_param);
t_list							*handle_param(t_token **tok, t_token *token,
...
void							close_fd(int fd1, int fd2, char *file_path,
									t_tree *simple_command);
void							handle_successful_read(
									t_successful_read_params *params);
void							init_read_params(
									t_successful_read_params *params,
									int new_hd_fd, int fd, char *new_hd);
