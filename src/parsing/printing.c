...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/04 18:36:07 by tischmid          #+#    #+#             */
/*   Updated: 2024/07/04 18:42:06 by tischmid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (tree->type != TOKEN)
	{
		if (tree->type == SIMPLE_COMMAND)
			ft_printf("- <%s> (%d children, in: %d, out: %d, err: %d)\n",
				...
				tree->children->len, tree->fd_in, tree->fd_out, tree->fd_err);
		else if (tree->type == PIPE_SEQUENCE)
			ft_printf("- <%s> (%d children, negated: %s)\n",
				tree_type_to_string(tree->type), tree->children->len,
		...
		ft_printf("- %s (\033[31m%s\033[m, splitnum: %d, is_last: %s, "
			"origin: %s)\n", token_type_to_string(tree->token->type),
			tree->token->str, tree->token->num_tokens_after_split,
